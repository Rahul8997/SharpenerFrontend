...
describe('Expense Component', () => {
    ...
    test('testing Enter your expenseamount: word', () => {
        render(<Provider store={store}><Expenses /></Provider>);
        const expenseWord = screen.getAllByText('expenseamount:',{exact:false});
        expect(expenseWord).not.toHaveLength(0);
    })
    ...
    test('testing Amount word', () => {
        render(<Provider store={store}><Expenses /></Provider>);
        const expenseWord = screen.getAllByText('Amount',{exact:false});
        expect(expenseWord).not.toHaveLength(0);
    })
